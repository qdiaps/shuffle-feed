...
def load_config():
    ...
    if not api_id:
        logger.error("Ошибка: API_ID не найдено в .env", exc_info=True)
        raise ValueError("API_ID не найдено в .env")

    api_hash = getenv("API_HASH")
    if not api_hash:
        logger.error("Ошибка: API_HASH не найдено в .env", exc_info=True)
        raise ValueError("API_HASH не найдено в .env")

    bot_token = getenv("BOT_TOKEN")
    if not bot_token:
        logger.error("Ошибка: BOT_TOKEN не найдено в .env", exc_info=True)
        raise ValueError("BOT_TOKEN не найдено в .env")

    super_admin_id = getenv("SUPER_ADMIN_ID")
    if not super_admin_id:
        logger.error("Ошибка: SUPER_ADMIN_ID не найдено в .env", exc_info=True)
        raise ValueError("SUPER_ADMIN_ID не найдено в .env")

    db_name = getenv("DB_NAME")
    if not super_admin_id:
        logger.error("Ошибка: DB_NAME не найдено в .env", exc_info=True)
        raise ValueError("DB_NAME не найдено в .env")

    db_timeout = getenv("DB_TIMEOUT")
    if not db_timeout:
        logger.error("Ошибка: DB_TIMEOUT не найдено в .env", exc_info=True)
