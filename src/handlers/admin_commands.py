...
async def cmd_admin_help(message: Message):
    logger.debug("Вывод админских команд.")
...
async def cmd_add_admin(message: Message, command: CommandObject, bot: Bot):
    ...
    if not command.args:
        await message.answer("/add_admin [user id]\n\nНапример: /add_admin 861583827")
        return
    ...
    try:
        ...
        logger.warning(
            f"Не вышло отправить оповещение юзеру {new_admin_id}: {e}", exc_info=True
        )
...
async def cmd_remove_admin(message: Message, command: CommandObject, bot: Bot):
    ...
    if target_id == int(config.SUPER_ADMIN_ID):
        ...
        logger.info(
            f"Админ {message.from_user.id} пытается забрать забрать админку у super admin {config.SUPER_ADMIN_ID}."
        )
        return
...
async def cmd_add_channel(
    ...
    (
        ...
        title,
        channel_int_id,
    ) = await parser.check_channel_and_get_preview(raw_username)

    if not success:
        await message.answer(f"Ошибка: {preview_ids}")
        logger.error(
            f"Ошибка при парсинге канала {raw_username}: {preview_ids}", exc_info=True
        )
        return
    ...
    for msg_id in reversed(preview_ids):
        try:
            await bot.copy_message(
                chat_id=message.chat.id, from_chat_id=target_chat_id, message_id=msg_id
            )
        except Exception:
            logger.warning(
                f"Не вышло скопировать сообщение {msg_id} с канала {raw_username} ({target_chat_id})."
            )

            file_path, caption, media_type = await parser.download_media_from_post(
                raw_username, msg_id
            )
            ...
            if file_path:
                try:
                    ...
                    if media_type == "video":
                        await message.answer_video(
                            video=video_file, caption=caption, parse_mode="Markdown"
                        )
                    elif media_type == "photo":
                        await message.answer_photo(
                            photo=video_file, caption=caption, parse_mode="Markdown"
                        )
                except Exception as e:
                    await message.answer(f"Не удалось загрузить файл: {e}")
                    logger.error(
                        f"Ошибка при загрузке файла {file_path}: {e}", exc_info=True
                    )
                finally:
                ...
                await message.answer(
                    f"<a href='https://t.me/{raw_username}/{msg_id}'>Пост #{msg_id}</a> (Бот не смог скопировать)",
                    parse_mode="HTML",
                )
        await asyncio.sleep(1)

    await message.answer(
        f"Найдено: {title} (@{raw_username})\nВсё верно? Добавляем в базу?",
...
@router.callback_query(
    AddChannelState.waiting_for_confirmation, F.data == "confirm_add_channel"
)
async def process_confirm(callback: CallbackQuery, state: FSMContext):
    if not isinstance(callback.message, Message):
        reason = (
            "InaccessibleMessage (Удалено?)"
            if callback.message
            else "None (Инлайн режим?)"
        )

        logging.warning(
            f"Не удалось отредактировать сообщение в колбэке ({reason}): user_id = {callback.from_user.id}, data = {callback.data}"
        )

        await callback.answer("Сообщение устарело или недоступно.", show_alert=True)
        return
...
@router.callback_query(
    AddChannelState.waiting_for_confirmation, F.data == "cancel_add_channel"
)
...
async def process_delete_request(callback: CallbackQuery, bot: Bot):
    ...
    if not callback.data:
        logging.warning(
            f"Получен пустой callback.data: user_id = {callback.from_user.id}, message_id = {callback.message.message_id if callback.message else 'Unknown'}"
        )

        await callback.answer("Ошибка кнопки (нет данных).")
        return

    try:
        _, channel_username, raw_msg_id = callback.data.split(":")
    ...
    if is_admin or callback.from_user.id == int(config.SUPER_ADMIN_ID):
        ...
        if deleted:
            ...
            logger.info(
                f"Админ {callback.from_user.id} удалил пост {msg_id} канала {channel_username}"
            )
        else:
    ...
    if not admins:
        await callback.answer(
            "Админы не найдены, некому жаловаться :(", show_alert=True
        )
        return
    ...
    report_text = (
        f"Жалоба на пост\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"От: {callback.from_user.full_name} {'@' + callback.from_user.username if callback.from_user.username != None else '[ Нет username ]'} (ID: {callback.from_user.id})\n"
    ...
    for admin_id in admins:
        try:
            ...
            logger.error(
                f"Не удалось отправить репорт админу {admin_id}: {e}", exc_info=True
            )
...
async def process_admin_decision(callback: CallbackQuery):
    ...
    if decision == "no":
        ...
        if is_deleted:
            ...
            await callback.answer(
                "Кто-то другой (или вы) уже удалил этот пост.", show_alert=True
            )
...
async def cmd_stats(message: Message, bot: Bot):
    ...
    text = f"Администраторы ({len(admins_list)}):\n"
    ...
    text = f"Каналы ({len(channels_data)}):\n"
    ...
    try:
        ...
        if os.path.exists(archive_path):
            await message.answer_document(
                document=FSInputFile(archive_path), caption="Полный архив логов."
            )
        else:
